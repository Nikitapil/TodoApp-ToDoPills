...
function addItem() {
    ...
    let data = new Date()
    let dataContent = data.getDate() + "." + (data.getMonth()+1) + "." + data.getFullYear()
...
todoContainer.addEventListener("click", function (event){
    if (event.target.classList.contains("inprogress__button")) {
        ...
        if (event.target.closest(".todo__item").parentElement.classList.contains("todo__trash__content")){
        ...
        event.target.closest(".todo__block").querySelector(".todo__content").innerHTML =""
        localStorage.setItem("elements", todoContainer.innerHTML)
            counters()
    }
})
...
if (event.target.classList.contains("item__show")) {
    let itemTitle = event.target.closest(".todo__item").querySelector(".todo__item__header").textContent
    let itemdescription = event.target.closest(".todo__item").querySelector(".todo__item__description").textContent
    editModal.innerHTML = `
    <h2>${itemTitle}</h2>
            <p>${itemdescription}</p>
    ...
    savelEditModal.addEventListener("click", function(){
        event.target.closest(".todo__item").querySelector(".todo__item__header").textContent = editHeaderInput.value
        event.target.closest(".todo__item").querySelector(".todo__item__description").textContent = editCommentInput.value
        localStorage.setItem("elements", todoContainer.innerHTML)
        showModal(editModal)
    })
    }
